...
export const TeamSelectorButton = ({ logo, label, onClick, className = '' }) => {
  return (
    <button 
      onClick={onClick}
      className={`flex flex-row justify-between items-center rounded bg-button-bg px-3 py-2 gap-2 flex-1 h-8 min-w-0 max-w-full ${className}`}
    >
      {/* Frame 805 */}
      ...
      <div className="relative w-4 h-4 flex-shrink-0">
        <img src={updown} alt="updown" className="w-4 h-4" />
      </div>
    </button>
  );
};
